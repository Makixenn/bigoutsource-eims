...
export default function SetupPassword() {
  ...
  useEffect(() => {
    async function verifyToken() {
      ...
      try {
        const result = await authService.verifySetupPasswordToken(token);
  ...
  const handlePasswordSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to initialize password setup');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isVerifyingToken) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#F9FAFB]">
        ...
        <p className="text-sm font-medium text-[#6B7280]">Verifying secure link...</p>
      </div>
    );
  }
            ...
            {!isTokenValid ? (
              <>
                ...
                <p className="text-[#6B7280] text-sm mt-3 text-center text-balance leading-relaxed cursor-default select-none">
                  This password setup link is invalid, has expired, or has already been used to customize a password.
                </p>
              </>
            ) : mfaToken ? (
              <>
                ...
                <p className="text-[#6B7280] text-sm mt-3 text-center text-balance leading-relaxed cursor-default select-none">
                  We've sent a 6-digit verification code to your email. Enter it below to complete password setup and log in.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-[#6B7280] text-sm mt-3 text-center text-balance leading-relaxed cursor-default select-none">
                  Welcome to EIMS! Please customize your password to activate your secure account access.
                </p>
              </>
            )}
          </div>
              ...
              <div className="space-y-6">
                <div className="flex flex-col items-center p-6 border border-red-100 bg-red-50/50 rounded-2xl">
                  ...
                  <p className="text-xs text-center text-red-600 font-semibold leading-relaxed">
                    For security reasons, password setup links are single-use only. If you have already set your password, please log in directly using your email and password.
                  </p>
                </div>
                <button
                  type="button"
